...
__global__ void SPMV(int NUM_THREAD, int NUM_TILE, int num_row, int j, int k, int ix, int y, int r,
	...
	if (idx < NUM_THREAD)
	{
		...
		for (n = startn; n < endn; n++)
		{
			...
			for (m = startm; m < endm; m++)
			{
				for (i = 0; i < r; i++)
				{
					atomicAdd(&D[(startD + rows[m]) * r + i], values[m] * C[(Ci[n] / y - k) * r + i] * B[(Ci[n] % y - j) * r + i]);
				}
			}
		}
	}
...
__host__ void hpsptm(int num_row, int num_column, int nnz, int NUM_SUBBLOCK, int NUM_THREAD, int NUM_SUBBLOCKperTHREAD, int NUM_TILE, int NUM_NECOL, int y, int z, int r, int ix, int iy, int iz, int *numnonpertile_perm, int *rows, int *columns, int *Cp, int *Ci, int *PtoNUM_NECOlperTILE, double *values, double *B, double *C, double *D, double *D_host)
{
	int i, j, k, q, ib;
	int iyy, izz;
	for (i = 0; i < (y * r); i++)
	...
	cudaMalloc((double **)&D_device, ix * NUM_THREAD * NUM_SUBBLOCKperTHREAD * r * sizeof(double));
	...
	for (ib = 0; ib < NUM_SUBBLOCKperTHREAD; ib++)
	{
		...
		cudaMemcpyAsync(D_device, D, ix * NUM_THREAD * r * sizeof(double), cudaMemcpyHostToDevice, stream0);
		...
		while (j < y)
		{
			...
			cudaMemcpyAsync(B_device, B + (j * r), (iyy * r) * sizeof(double), cudaMemcpyHostToDevice, stream0);
			...
			while (k < z)
			{
				cudaMemcpyAsync(C_device, C + (k * r), (izz * r) * sizeof(double), cudaMemcpyHostToDevice, stream0);
				...
				if (izz >= 2)
				{
					for (i = 0; (i + 2) <= izz; i += 2)
					{
						...
						SPMV<<<grid, block, 0, stream0>>>(NUM_THREAD, NUM_TILE, num_row, j, k, ix, y, r,
						...
						SPMV<<<grid, block, 0, stream1>>>(NUM_THREAD, NUM_TILE, num_row, j, k, ix, y, r,
		...
		cudaMemcpyAsync(D_host + (ib * ix * NUM_THREAD * r), D_device, ix * NUM_THREAD * r * sizeof(double), cudaMemcpyDeviceToHost, stream0);
...
int main(int argc, char **argv)
{
	...
	scanf("%d %d %d", &y, &z, &r);
	int ix, iy, iz, NUM_THREAD;
	printf("Please Input ix, iy, iz, and NUM_THREAD:\n");
	scanf("%d %d %d %d %d", &ix, &iy, &iz, &NUM_THREAD);
	...
	hpsptm(num_row, num_column, nnz, NUM_SUBBLOCK, NUM_THREAD, NUM_SUBBLOCKperTHREAD, NUM_TILE, NUM_NECOL, y, z, r, ix, iy, iz, numnonpertile_perm, rows, columns, Cp, Ci, PtoNUM_NECOlperTILE, values, B, C, D, D_host);
