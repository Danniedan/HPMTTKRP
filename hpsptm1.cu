		...
		for(n=startn; n<endn; n++)
		{
			...
			for(m=startm; m<endm; m++)
			{
				for(i=0; i<ir; i++)
				{
					...
					//printf("idx=%d, values[%d]=%lf, C[%d]=%lf, B[%d]=%lf, D[%d]=%lf\n", idx, m, values[m], 
						//(Ci[n]/y-k)*ir+i, C[(Ci[n]/y-k)*ir+i], (Ci[n]%y-j)*ir+i, B[(Ci[n]%y-j)*ir+i], (startD+rows[m])*ir+i, D[(startD+rows[m])*ir+i]);
				}
			}
		}
...
int main(int argc, char** argv)
{
	...
	/*****************************************************************/
	//char * filename="/root/cyd/data/user_ratedmovies_reduce.txt";
	...
	for(b=0; b<NUM_TILE; b++)
	{
		...
		//printf("b=%d, numnonpertile_perm=%d\n", b, numnonpertile_perm[b+1]-numnonpertile_perm[b]);
				...
				while(k<z)
				{
					...
					if(izz>=2)
					{
						for(i=0; (i+2)<=izz; i+=2)
						{
							...
							//printf("mark[%d]=%d, mark[%d]=%d\n", q*NUM_THREAD, PtoNUM_NECOlperTILE[q*NUM_THREAD], (q+1)*NUM_THREAD, PtoNUM_NECOlperTILE[(q+1)*NUM_THREAD]);
							...
							//printf("mark[%d]=%d, mark[%d]=%d\n", (q+1)*NUM_THREAD, PtoNUM_NECOlperTILE[(q+1)*NUM_THREAD], (q+2)*NUM_THREAD, PtoNUM_NECOlperTILE[(q+2)*NUM_THREAD]);
